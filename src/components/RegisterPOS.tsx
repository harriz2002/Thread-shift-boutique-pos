...
export const RegisterPOS: React.FC<RegisterPOSProps> = ({
  ...
  const renderCart = (showCloseButton = false, onClose?: () => void) => {
    return (
      <div className="flex flex-col justify-between h-full space-y-4">
        ...
        {/* Cart Itemized List */}
        <div className="flex-1 overflow-y-auto max-h-[380px] lg:max-h-[calc(100vh-420px)] space-y-2.5 pr-1 no-scrollbar">
          {cart.length === 0 ? (
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 p-4 sm:p-6 max-w-7xl mx-auto relative">
      ...
      {/* Right Area: Active Cart Drawer (5 Cols) for Desktop */}
      <div className="hidden lg:flex lg:col-span-5 bg-slate-900 border border-slate-800 rounded-2xl p-4 flex-col justify-between shadow-xl space-y-4">
        {renderCart(false)}
      </div>

      {/* Floating Sticky Cart Toggle Button for Mobile Screens */}
      <div className="lg:hidden fixed bottom-6 right-6 z-40">
        <button
          onClick={() => setIsCartOpen(true)}
          className="flex items-center gap-2.5 px-5 py-3.5 bg-amber-500 hover:bg-amber-400 text-slate-950 font-extrabold rounded-full shadow-2xl shadow-amber-500/35 active:scale-95 transition-all border border-amber-400/20"
        >
          <div className="relative">
            <ShoppingBag className="w-5 h-5 shrink-0" />
            {cart.length > 0 && (
              <span className="absolute -top-2.5 -right-2.5 bg-rose-500 text-white font-extrabold text-[10px] w-5 h-5 rounded-full flex items-center justify-center border border-slate-950 shadow-sm animate-pulse">
                {cart.reduce((a, b) => a + b.quantity, 0)}
              </span>
            )}
          </div>
          <span>View Cart ({formatCurrency(grandTotal)})</span>
        </button>
      </div>

      {/* Mobile Slide-over Drawer Backdrop & Panel */}
      {isCartOpen && (
        <div className="fixed inset-0 z-50 lg:hidden flex justify-end bg-slate-950/80 backdrop-blur-sm transition-opacity duration-300 animate-in fade-in">
          {/* Click outside to close */}
          <div className="absolute inset-0" onClick={() => setIsCartOpen(false)} />
          
          {/* Cart Panel Slide-over Drawer */}
          <div className="relative w-full max-w-md bg-slate-900 border-l border-slate-800 h-full p-5 flex flex-col justify-between shadow-2xl z-10 animate-in slide-in-from-right duration-300">
